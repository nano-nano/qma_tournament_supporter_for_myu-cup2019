<template>
  <div class="semiFinal-screen">
    ...
    <twitter-select-dialog ref="twitterSelectDialog" roundName="F"
...
export default {
  ...
  methods: {
    projection () {
      this.$store.commit('updateProjectionScreen', this.$router.resolve('finalprojection').href)
    },
    ...
    loadRoundPlayersData () {
      FileUtils.loadAllPlayersData().then((loadData) => {
        ...
        const nPlayers = loadData.filter((e) => { return e.roundDatas['F'] != undefined }).length
        for (let set = 1; set <= (Math.ceil(nPlayers / 4)); set++) {
          for (let seat = 1; seat <= 4; seat++) {
            const extracted = PlayerUtils.extractPlayerDataBySetSeatNo(loadData, 'F', set, seat)
            const roundData = extracted.roundDatas['F']
        ...
        for (let importedData of importedDataArray) {
            for (let parentData of this.extractedPlayersData) {
                if (parentData.cardName == importedData.cardName) {
                    ...
                    targetSetNo = parentData.roundDatas['F'].setNo
                    break
                }
            }
        }
    ...
    saveScore (setNo, isNeedDialog) {
      ...
      ScoreUtils.updateScore(targetPlayersData, 'F').then((updatedData) => {
  ...
  mounted: function () {
    this.$store.commit('updateCurrentScreen', 'final')
